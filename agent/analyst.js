...
export function analyze(observation) {
  ...
  for (const result of observation.results) {
    ...
    const category = result.dependency === 'external' ? 'external_dependency'
      : result.errorCode === 'region_order_mismatch' ? 'ui_regression'
        : result.errorCode === 'content_type_mismatch' || result.errorCode === 'asset_version_mismatch' ? 'asset_integrity' : 'connectivity';
